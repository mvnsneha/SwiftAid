...
function EventTimeline() {
  ...
  // Fake weather event (you can connect real API later)
  ...
  useEffect(() => {
    const loadEvents = async () => {
      ...
      setEvents(combined.slice(0, 8)); // latest 8
    };

    loadEvents();

    const interval = setInterval(loadEvents, 60000); // refresh every 1 min
    return () => clearInterval(interval);
  }, []);

  return (
    <section style={{ ...styles.container, backgroundColor: "#060933" }}>
      <h2 style={{ ...styles.title, color: "rgb(221, 221, 230)" ,fontSize: "35px" }}>
        🕒 Event Timeline
      </h2>
          ...
          <div key={index} style={styles.item}>
            ...
            <div
              style={{
                ...styles.dot,
                background:
                  event.type === "earthquake" ? "red" : "#38bdf8",
              }}
            ></div>
